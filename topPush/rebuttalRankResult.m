    ...
    for round = 1:total_round
        ...
        for i_tol = 1:size(ts, 2)
            ...
            if SVMpAUC
                scale = 1.01;
                SVMpAUC_opt = [];
                SVMpAUC_opt.scale = scale;
                SVMpAUC_opt.n_fold = n_fold;
                SVMpAUC_opt.target = target;
                SVMpAUC_opt.t = tolerance;
                %this option is only used when n_fold == 1
                SVMpAUC_opt.valid_rate = 1/3;
                SVMpAUC_opt.lambda_lb = 1e-3;
                SVMpAUC_opt.lambda_ub = 1e3;
                SVMpAUC_opt.lambda_factor = step_size;
                SVMpAUC_opt.seed = round;
                [best_lambda, best_target] = SVMpAUC_GridSearch(Xtr, ytr, SVMpAUC_opt); 
                fprintf('Best lambda: %f, best_target: %f.\n', best_lambda, best_target);

                if strcmp(target, 'T_AUC')
                    t_left = 0;
                    t_right = max(tolerance, 1/sum(ytr==-1) * scale);
                elseif strcmp(target, 'BTPR')
                    t_left = max(0, tolerance - 1/sum(ytr==-1) * scale);
                    t_right = max(tolerance, 1/sum(ytr==-1) * scale);    
                end
