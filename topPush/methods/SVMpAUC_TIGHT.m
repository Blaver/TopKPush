function w = SVMpAUC_TIGHT(X, y, alpha_val, beta_val, C, eps, verbosity)
        ...
        if(n*(beta_val - alpha_val) < 1)
            disp(n);
            disp(n*(beta_val - alpha_val));
            fuck_result = n*(beta_val - alpha_val)
...
function delta = loss_pAUC(a)
        ...
        if(neg_alpha_ceil == 0)
            delta = sum(m - aminus(1:neg_beta_floor));
            if(neg_beta_floor < neg_beta_ceil)
                delta = delta + (sbeta*m - aminus(neg_beta_ceil));
            end
        else
            delta = salpha*(m - aminus(neg_alpha_ceil)) + sum(m - aminus(neg_alpha_ceil_next:neg_beta_floor)); 
            if(neg_beta_floor < neg_beta_ceil)
                delta = delta +  (sbeta*m - aminus(neg_beta_ceil));
            end
        end

        delta = delta / m / n / (beta - alpha); % Normalize
end
...
function psi = joint_feature_map(x, a, neg_subset)
        ...
        psi = psi' / m / n / (beta - alpha); % Normalize
end
...
function w = cutting_plane(dataset, C, epsilon)
        ...
        options = optimset('Display','off','Algorithm','interior-point-convex','MaxIter',1000);
        ...
        while(1)
            ...
            loss(ccnt+1,:) = loss_pAUC(most_violated_ordering);
